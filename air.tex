...
threshold used to determine precipitation phase and the ice emissivity. Model calculations estimate that the
Schwarzsee Icestupa only stored about 8\% of the total water sprayed as ice. In addition, we found that reducing
nozzle diameter of the fountain to 3 $mm$ increases the storage efficiency up to 93\% without compromising on the
storage duration.
...
\subsection{Construction} From $30^{th}$ January to $18^{th}$ March 2019 the Icestupa was constructed through the fountain spray, which was
...
ultrasonic wind sensor was blocked and recorded zero values. These data gaps and errors were filled with the ERA5
reanalysis dataset. ERA5 dataset is produced at a 1-hourly time step. Its horizontal resolution is approximately 30 km
and it computes atmospheric variables at 137 pressure levels \citep{ERA5_2020}. The ERA5 grid point chosen(Latitude
46.640, Longitude 7.240) for the Schwarzsee site was around 9 km away from the actual site. So in order to fit the
corresponding variables in the Schwarzee dataset, several modifications were necessary. First, the hourly ERA5 data
were linearly interpolated to the 5 minute data frequency of the Schwarzsee AWS. Then, selected variables of the ERA5
dataset were matched with Schwarzsee variables as follows: 

\begin{itemize} 
  \item Precipitation: Since no precipitation measurements were made in Schwarzsee, the nearby Plaffein
    weather station was used for precipitation data instead as shown in Fig. \ref{fig:input}.
  \item Air temperature: This variable correlated well ($r^2=0.73$) with 2m temperature of ERA5. It was further corrected
    using the following equation:
    \begin{equation}
      f_{corr}(x,y) = y + sgn(rmse(x,y)) \cdot rmse(x,y)
    \end{equation}
    where $f_{corr}$ was the correction function used, $x$ was the Schwarzsee temperature dataset ($T_{SZ}$), $y$ was
    the ERA5 temperature dataset ($T_{ERA5}$), $sgn$ is the signum function  and $rmse$ was the root mean square error
    function defined as: $$rmse(x,y) = \sqrt{(\frac{1}{n})\sum_{i=1}^{n}(y_{i} - x_{i})^{2}}$$
  \item Global and diffuse radiation: Since this variable was not measured in Schwarzsee, ERA5 surface solar radiation
    downwards was used as a proxy for global radiation and total sky direct solar radiation was subtracted from it
    to derive the diffuse radiation shown in Fig. \ref{fig:input}.
  \item Relative Humidity: Near-surface humidity is not archived directly in ERA datasets, but 
    from near-surface (2m from the surface) temperature ($T_{ERA5}$) and dew point temperature ($Td_{ERA5}$)
    one can calculate relative humidity($RH$) at 2m as follows:
     \begin{equation}
       RH = 100 \cdot \frac{e_{sat}(Td_{ERA5})}{e_{sat}(T_{ERA5})}
      \end{equation}
      where the saturation vapour pressure function $e_{sat}$ is expressed with the Teten's formula \citep{Tetens}:
\begin{equation}
  e_{sat}(T)= a_1 \cdot e^{(a_3 \cdot \frac{T}{(T+273.16-a_4)})} 
\end{equation}
with T in $\degree C$ and the parameters set for saturation over water ($a_1$ = 611.21 Pa, $a_3$ = 17.502
and $a_4$= 32.19 K) according to \cite{Buck_1981}. This derived variable was poorly correlated ($r^2=0.38$) with the
Schwarzsee measurements.
  \item Air Pressure: This variable correlated well ($r^2=0.99$) with surface pressure of ERA5. It was further modified
    using the correction function $f_{corr}(T_{SZ},T_{ERA5})$.
  \item Wind Speed: This variable correlated poorly($r^2=0.49$) with 10m wind speed of ERA5. We applied the
    $f_{corr}(T_{SZ},T_{ERA5})$ function to better fit ERA5 to Schwarzsee. There was another cause of missing wind
    speed data as the ultrasonic wind sensor recorded zero wind speed values due to snow accumulation on it. So all
    zero wind speed values were also replaced with the corrected ERA5 data. 
  \item Cloudiness index: Since this variable was not measured in Schwarzsee, ERA5 total cloud cover was used as a
proxy instead.
\end{itemize}

Later, we further extended the Schwarzsee dataset beyond $18^{th}$ March 2019 using this modified ERA5 dataset to
enable the model to completely melt the Icestupa.
...
The net longwave radiation $q_{LW}$, for which there were no direct measurements available at Schwarzsee, is
...
equation suggested by \cite{Brutsaert_1982}, considering air temperature and vapor pressure (Eqn.  \ref{eqn:atm_e}).
The vapor pressures over air and ice was obtained using the following formulation given in \cite{WMO_2018}:
...
(first term in equation \ref{eqn:atm_e}) is extended with the correction for cloudy skies after \cite{Brutsaert_1982}
as follows:
...
\subsubsection{Bulk Icestupa heat flux $q_{G}$} \label{sec:Bulkflux} The bulk Icestupa heat flux $q_{G}$ corresponds
to the ground heat flux in normal soils and is caused by the temperature gradient between the surface layer and the
ice body. It is expressed by using the heat conduction equation as follows:
...
\begin{equation} (q_{T}, q_{melt}) = \left\{ \begin{array}{ll} ( \rho_{ice} \cdot c_{ice} \cdot  \Delta x \cdot
    ...
    & \textit{ if } T_{ice}^{i+1}\leq 0 \textit{ and } \Delta M_{F} = 0\\
        % ( \rho_{ice} \cdot c_{ice} \cdot  \Delta x \cdot \frac{-T_{ice}^{i-1}}{\Delta t}, q_{net}-q_{L}) & \textit{
    % if } T_{ice}\leq 0 \textit{ and } (q_{net}-q_{L}) < 0 \textit{ and } \Delta M_{F} < -(q_{net}-q_{L})\cdot A
    % \cdot \Delta t/L_f  \\ ( \rho_{ice} \cdot c_{ice} \cdot  \Delta x \cdot \frac{\Delta T_{ice}}{\Delta t} , 0) &
...
%    \begin{tabular}{*5l}
    \begin{tabularx}{\linewidth}{ X l X l X  } \hline Constant Parameters & Symbol & Value & Range  & References \\
      ...
      constant & $\sigma$ & $5.67 \cdot 10^{-8} W\, m^{-2}\, K^{-4}$& n.a.   & B: \citeauthor{CuffeyPaterson_2010}\\
      Air pressure at sea level & $p_{0,a}$ & 1013 $hPa$ & n.a.   & B: \citeauthor{MolgHardy_2004}\\ Density of water
                                & $\rho_{w}$ & 1000 $kg\, m^{-3}$ & n.a.   & B: \citeauthor{CuffeyPaterson_2010}\\
      Density of ice & $\rho_{ice}$ & 917 $kg\, m^{-3}$ & n.a.   & B: \citeauthor{CuffeyPaterson_2010}\\ Density of
      air & $\rho_{a}$ &  1.29 $kg\, m^{-3}$ & n.a.   & B: \citeauthor{MolgHardy_2004}\\ Specific heat of water &
      $c_{w}$ & 4186 $J\, kg^{-1}\,\degree C^{-1}$ & n.a.   & B: \citeauthor{CuffeyPaterson_2010}\\ Specific heat of
      ice & $c_{ice}$ & 2097 $J\, kg^{-1}\,\degree C^{-1}$ & n.a.   & B: \citeauthor{CuffeyPaterson_2010}\\ Specific
      heat of air & $c_{a}$ & 1010 $J\, kg^{-1}\,\degree C^{-1}$ & n.a.   & B: \citeauthor{MolgHardy_2004}\\ Thermal
      conductivity of ice & $k_{ice}$ & 2.123  $W\, m^{-1}\, K^{-1}$ & n.a. & B: \citeauthor{Bonales_2017} \\ Latent
      Heat of Sublimation & $L_{s}$ & 2848 $kJ\, kg^{-1}$ & n.a.   & B: \citeauthor{CuffeyPaterson_2010}\\ Latent Heat
      of Evaporation & $L_{e}$ & 2514 $kJ\, kg^{-1}$ & n.a.   & B: \citeauthor{CuffeyPaterson_2010}\\ Latent Heat of
      Fusion & $L_{f}$ & 334 $kJ\, kg^{-1}$ & n.a.   & B: \citeauthor{CuffeyPaterson_2010}\\ Gravitational
      acceleration & $g$ & 9.81 $m\, s^{-2}$ & n.a.   & B: \citeauthor{CuffeyPaterson_2010}\\ \hline Uncertain
      Parameters& & &  & \\ \hline Precipitation Temperature threshold & $T_{rain}$ & 1 \degree C & $\pm$ $1\,\degree
      C$ & B + U: \citeauthor{FujitaAgeta_2000}, \citeauthor{Zhou_2010}\\
        
        Ice Emissivity        & $\epsilon_{ice}$ & 0.95 & $\pm$ 5 \% & B: \citeauthor{CuffeyPaterson_2010}; U:
        assum.\\ Ice Albedo         & $\alpha_{ice}$ & 0.35 & $\pm$ 5 \%  & B: \citeauthor{CuffeyPaterson_2010}; U:
        assum.   \\
...
\begin{equation} M_{F} + M_{ppt} + M_{dpt} = M_{ice} + M_{melt} + M_{vapour} + M_{drained} \label{eqn:M}
\end{equation}
...
$M_{melt}$ is the cumulative mass of melt water; $M_{vapour}$ represents the cumulative water vapor loss by
evaporation or sublimation and $M_{drained}$ is the cumulative water that drains away.
...
The model was forced with meteorological data from $30^{th}$ January to $1^{st}$ April 2019\textbf{} (Fig.
...
\begin{table} \caption{Summary of calculated mass balance components for the Schwarzsee experiment after the fountain
    ...
    \parbox{2mm}{\rotatebox[origin=c]{90}{ Input }} & $M_{ppt}$ & 439 $kg$ & 463 $kg$\\ & $M_{dpt}$ & 14 $kg$ & 62
    $kg$ \\ \hline & $M_{melt}$ & 166 $kg$ & 1392 $kg$\\ \parbox{2mm}{\rotatebox[origin=c]{90}{Output }} & $M_{ice}$ &
...
to the relatively small volume of the Schwarzsee Icestupa. In summary, more validation measurements on a more
