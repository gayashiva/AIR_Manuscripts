...
surface roughness, $z_{0}$, of glacier ice are generally in the range $0.1-5\, mm$ \cite{BrockWillisSharp_2006}.
...
\begin{table}
    ...
    Van Karman constant & $\kappa$      & 0.4        & & \citeauthor{CuffeyPaterson_2010}              \\
    ...
    & & &  & \\
		\textbf{Uncertain Parameters} & \textbf{Symbol} & \textbf{Range} & \textbf{Unit} & \textbf{References} \\\midrule
    Ice Emissivity                      & $\epsilon_{ice}$      & $[0.95,0.99]$         & & \citeauthor{HORI2006486}             \\
    Ice Albedo                          & $\alpha_{ice}$        & $[0.15,0.35]$         &  &
    \citeauthor{steiner_2015}; \citeauthor{ZollesMaussion_2019}           \\
    Snow Albedo                         & $\alpha_{snow}$       & $[0.8,0.9]$        &  & \citeauthor{ZollesMaussion_2019}              \\
    ...
    & &    &  & \citeauthor{OerlemansKnap_1998}      \\
    Surface Roughness                   & $z_0$                 & $[1,5]$            & $mm$  & \citeauthor{BrockWillisSharp_2006}       \\
    Surface Area correction factor      & $A_{corr}$            & $[1,2]$            &       & assumed       \\
    Surface layer thickness             & $\Delta x$            & $[10,50]$           & $mm$ & assumed
    \\\bottomrule
		% \textbf{Misc Parameters}    & \textbf{Symbol} & \textbf{Value} & \textbf{Unit} & \textbf{Section} \\\midrule
  %   Model timestep & $\Delta t$ & 3600 & $s$ & \\
  %   Weather station height & $h_{AWS}$ & 2 & $m$ & \\
  %   Ice body and surface distance & $l_{cone}$ &  &  & \ref{sec:Bulkflux}\\
    % Mass of fountain discharge & $M_{F}$ &  & $kg$ & \\
    % Mass of precipitation & $M_{ppt}$ &  & $kg$ & \\
    % Mass of deposition & $M_{dep}$ &  & $kg$ & \\
    % Mass of ice & $M_{ice}$ &  & $kg$ & \\
    % Mass of meltwater & $M_{water}$ &  & $kg$ & \\
    % Mass of sublimation & $M_{sub}$ &  & $kg$ & \\
    % Mass of fountain runoff& $M_{runoff}$ &  & $kg$ & \\
    % Radius of AIR & $r_{cone}$ &  &  & \ref{sec:shape}\\
    % Height of AIR & $h_{cone}$ &  &  & \ref{sec:shape}\\
    % Slope of AIR  & $s_{cone}$ &  &  & \ref{sec:shape}\\
    % Thickness change rate of AIR  & ${\Delta j_{cone}}/{\Delta t}$ &  &  & \ref{sec:shape}\\
	\end{tabular}
\end{table}
