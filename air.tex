...
to determine precipitation phase. Model calculations estimate that the Eispalast Icestupa stored about 6\% of the
...
speed values were recorded whenever snow accumulated on the ultrasonic wind sensor. All null values were replaced
...
mostly to the melt of the ice structure. Daily values of $q_{S}$ ranged from -15 to 26 $Wm^{-2}$. Turbulent latent heat
flux Mean of $q_{L}$ across model runtime was positive suggesting that it favoured deposition/condensation over
...
to 7 $Wm^{-2}$. The contribution of heat flux by conduction $q_{G}$ was minimal as it only varied between -7 to 8
...
Therefore, in the case of EP we used a water input of 18,560 $kg$, with a resultant storage efficiency of only 5.5 \%.
...
produced by these uncertain parameters in maximum ice volume calculation. It shows that $\epsilon_{ice}$ 
is the only parameters with a maximal sensitivity of more than 0.1 $m^3$ for the maximum ice volume estimate.
...
M_{F}}{\Delta t} < 0$) and the freeze rates (absolute growth rate when $\frac{\Delta M_{F}}{\Delta t} > 0$).
However, as the fountain was mostly inactive during the study period, the underestimation of surface area
...
to the relatively small volume of the EP Icestupa. In summary, more validation measurements on a more voluminous
Icestupa would have increased confidence on the model results.
...
Table \ref{table:MB} clearly shows that for our EP experiment most of the input water (92 \%) simply drained away
...
switched on is justified but a higher critical temperature could have been used in the case of the EP Icestupa. 
...
duration was around 3 days less than the 5 $mm$ nozzle. One physical cause of this effect is the different shapes of
...
the 3 $mm$ ice structure. Fig.  \ref{fig:dia_f} shows that a nozzle diameter of 3 $mm$ has an average freeze rate (2.4
