...
polynomial (as a surrogate model), on which sensitivity and uncertainty analysis can be performed. Throughout the
following analysis, we fix the model time resolution to be one hour.
...
($S_{T_{i}}$) of $< 0.05$, and these parameters were fixed at the median value of the range shown in Table
\ref{tab:parameters} in subsequent model simulations. Since all the AIR assume the same value and were very
sensitive to surface layer thickness $\Delta x$, it was calibrated first before performing any further calibration
...
The surface area correction factor, $A_{corr}$, surface roughness, $z_{0}$, and surface layer thickness, $\Delta
	x$, may need more attention. $A_{corr}$ quantifies the deviation of the AIR shape from the conical shape assumed in
the model. We assume the range of this deviation to be between 1 to 2. Literature values for surface roughness of
glacier ice are generally in the range $0.1-5\, mm$ \citet{BrockWillisSharp_2006}. Since the surface layer
thickness parameter for an AIR does not bear resemblance to any parameter in the glaciological literature, we
attribute a wide range of values for it and try to constrain it further during the calibration process.
...
\begin{table}
	...
	\begin{tabular}{@{}llllll@{}}
		...
		8            & Surface layer thickness             & $\Delta x$            & 1            & 5            & $cm$          \\\bottomrule
	\end{tabular}
\end{table}
...
rosenblatt transformation was also used since the parameters were correlated.
...
IN21 AIR does exhibit a strong preference for a surface layer thickness of $50\,mm$. Hence, this model parameter
...
Fig. \ref{fig:results} shows the model ice volume estimates with corresponding uncertainties caused by the
parameters and the fountain.

The construction decisions responsible for the observed magnitude and variance in the ice volume estimates can be
categorised based on the location chosen and the fountain used. The construction location chosen determines the
weather and the fountain usage is characterized by its four attributes.

\subsection{Fountain influence}

The fountain determines the surface area through its spray radius and also influences the energy fluxes through its
water temperature ($q_{F}$) and through its influence on albedo ($q_{SW}$). Considering the high correlation of
surface area on the ice volume estimates ($r^2=0.6$), we investigate the fountain influence through its impact on
the AIR surface area henceforth.

Fig. shows the estimated and observed surface area via the model and UAV respectively. Surface area has a low
variance during fountain runtime since ice radius is initialised as spray radius and bounded by it.

\subsection{Location influence}

Consequently, the weather influence can be analysed by its impact on the surface normal thickness change on the AIR
surface.

\subsection{Mass and energy fluxes}
