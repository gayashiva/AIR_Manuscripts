...
\subsubsection{Fountain spray radius}
The model requires the fountain spray radius to be provided for producing AIR ice volume estimates. This is a
significant limitation since the model is very sensitive to the spray radius parameter since the surface area is
directly proportional to it during the accumulation period. Moreover, the spray radius of the AIR is not only
determined by the fountain characteristics but also due to refreezing and melting events across the AIR
perimeter. Therefore, the same fountain may produce drastically different spray radius under different weather
conditions.
...
\subsubsection{Shape assumption}
 ...
 such a large error namely, assuming a conical shape and assuming a constant radius during the accumulation
 period.

 Both these assumptions are a consequence of favoring the model simplicity over accuracy. One could, for
 example, model the AIR shape assuming its cross section is a gaussian curve instead of a triangle.  But
 such methodologies will involve inclusion of more model parameters.

\subsection{Model sensitivity, calibration and validation}

The surface layer parameter is the only parameter compensating for the poor shape assumption of the model.
Therefore, we find it to be an outlier in our global sensitivity analysis. This is particularly true for the
IN21 AIR where in reality two AIRs merged leading to a drastically different shape evolution compared to our
model.   
...
radius is also biased as a consequence leading to further model error. This could be one of the reasons we
observe a poor validation with the CH20 AIR since the spray radius is derived from just one drone survey.
...
high water losses were caused by the fountain wastewater in both the AIRs. Since vapour losses were
...
not limited by the supply rate but by the freezing efficiency. The CH21 AIR freezing rate was able to reach the mean fountain
discharge provided, albeit for only 6 hours out of the 2155 hours of fountain runtime available. Thus, water
...
Moreover, the fountain parameters were not independent, since fountain height (ignored in this analysis),
discharge rate and spray radius were related through the trajectories of the water droplets. Therefore, a proper
optimization of the fountain is much more complex and requires a closer look at the correlation of the fountain
parameters amongst themselves and with the freezing/melting energy flux. This will be investigated in a follow
up study, with this study focusing on the weather aspects of the model.
...
The Indian location was found to be favourable to the formation of AIRs since its mean thickness change was positive. Particularly, the mean
...
\begin{equation} U \approx \hat{U}(j, Q) = \sum_{n=0}^{N_p-1} c_n(j) \phi_n(Q) \end{equation}
