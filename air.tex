...
\begin{table} \caption{Summary of mass balance components for the EP experiment after the fountain
  spray was stopped (on $15^{th}$ February 2019 ) and at the end of the model run (on $5^{th}$ April). All parameters
    ...
    & \multicolumn{1}{c|}{Model ends} \\ \hline & $M_F$ & 18060 $kg$ & 18060 $kg$\\
      \parbox{2mm}{\rotatebox[origin=c]{90}{ Input }} & $M_{ppt}$ & 444 $kg$ & 466 $kg$\\ & $M_{dpt}$ & 0 $kg$ & 0
      $kg$ \\ & $M_{cdt}$ & 7 $kg$ & 32 $kg$ \\
      \hline & $M_{melt}$ & 163 $kg$ & 1013 $kg$\\
      \parbox{2mm}{\rotatebox[origin=c]{90}{Output }} & $M_{ice}$ & 809 $kg$ & 0 $kg$\\ & $M_{vapour}$ & 11 $kg$ & 16
...
$q_{LW}$ accounted for 32.5\% of overall energy turnover. The energy turnover is calculated as the sum of energy
fluxes in absolute values. $q_{SW}$ accounted for 31.5\%, followed by $q_{melt}$ (20\%), $q_{S}$ (9.8\%), $q_{L}$
(3.2\%), $q_{G}$ (2\%), $q_{F}$ (0.2\%) and $q_{T}$ (0.4\%).
