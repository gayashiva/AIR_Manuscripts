...
where $q_{total}$ is represents the total energy available to be redistributed for the melting ($q_{melt}$),
freezing ($q_{freeze}$) and surface temperature change ($q_{T}$) processes.
...
the latent heat turbulent fluxes can only contribute to temperature fluctuations. Therefore, for preventing
latent heat energy from turning a melting event into a freezing event, an additional condition, namely,
$(q_{total}-q_{L}) < 0$, is required.
...
lower latitude and lower cloudiness. However, the energy turnover of the shortwave radiation for both sites
...
bottom of the surface layer. Therefore, additional spatial and temporal ice temperature measurements of the AIR
could have better constrained the range of the surface layer thickness parameter. The minimum modelled surface
temperature was actually similar for all our three AIRs around $-24\,\degree C$. Therefore, one could, for
example, calibrate $\Delta x$ to minimize the minimum observed versus modelled surface temperature ($T_{ice}$).  
...
high water losses were caused by the fountain wastewater in both AIRs. Therefore, AIRs waste water mostly
...
melting rates. Hence, AIRs need to be inculcated in the water resource management strategy particularly of dry
...
similar to the ones used on glaciers the world over, we believe it's performance should be similar in any other
location.  
...
for the future, it is conceivable to extend the AIR model for automatic AIR construction and build a community
platform around it where scientists and farmers can improve and use the model together. 
