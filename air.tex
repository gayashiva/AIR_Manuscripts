...
sites in Switzerland and one in India. At these measurement sites, AIR were built for model calibration and validation purposes. Model
estimates had a normalised root mean square error of 5 \% and a correlation of 0.9. Water storage efficiency of the AIR
varied widely not just between different locations but also between different winters.
...
The Gangles site (34.22 $\degree$N, 77.61 $\degree$E) is located around 20 km north of Leh city in the Ladakh region lies at 4025 $m$ a.s.l.. AIR were
constructed here by the Himalayan Institute of Alternatives, Ladakh (HIAL) every winter since 2018.
...
rough appearance and forms an obstacle to the wind regime. This formulation is inspired from \cite{Oerlemans_2021} but
here we define it much more generally as follows:  

\begin{equation}
    \mu_{cone} = 1 + \mu^* \cdot s_{cone}
\end{equation}

where $\mu^*$ is a constant between 0 to 1 called the turbulent flux parameter. In \cite{Oerlemans_2021}, $\mu^*$ is
assumed to be 0.5 and we use this as the default value of the parameter but later conduct a sensitivity analysis to
...
\begin{table} 
    \centering
    \caption{Free parameters in the model categorised as constant, surface and fountain parameters. Base
        value (B) and uncertainty (U) were taken from the literature. For measurements (meas.) and assumptions (assum.),
    the uncertainty was chosen to be relatively large (5 \%).}
\label{tab:parameters} 
    ...
    \begin{tabularx}{\linewidth}{ X l X l X  } 
        ...
        Ice Emissivity        & $\epsilon_{ice}$ & 0.97 & 0.949-0.993 & B + U : \citeauthor{steiner_2015}\\
        Ice Albedo         & $\alpha_{ice}$ & 0.15 & 0.01-0.35 & B + U : \citeauthor{steiner_2015} \\
        ...
        Surface Roughness & $z_0$ & 1 $mm$ & $1-5\, mm$ & B + U : \citeauthor{steiner_2015}\\ 
        % Turbulent flux parameter & $\mu^*$ & 0.5 & $[0,1]$ & B: \citeauthor{Oerlemans_2021}; U: assum.  \\ 
        Surface layer thickness & $\Delta x$ & 20 $\,mm$ & $\pm$ 5 \%   & B + U: assum.\\
...
\ref{fig:freeze_rate} shows CH21, IN21 and CH20 had a maximum freezing rate of just 7.5 $l/min$, 18 $l/min$ and 6 $l/min$
respectively compared to the 7.5 $l/min$, 60 $l/min$ and 7.5 $l/min$ of mean fountain discharge they were provided with.
...
% To understand the environmental influences associated with this growth rate, we 
   
% The model results for all the three AIR are presented in a two step manner. In the first step, we analyse the results of
% the CH21 AIR to understand the environmental influences involved in its evolution. Then in a second step, we compare it
% with the rest of the two AIR.

% \subsection{CH21 AIR} 
% The model was forced with meteorological data from $22^{nd}$ November to $10^{th}$ May 2021 (Fig.  \ref{fig:input}) and
% various parameters (see Table \ref{tab:parameters}) to calculate the mass and energy balance of the CH21 AIR.
