...
for filling data gaps and adding data that were not measured directly as shown in Fig. \ref{fig:input}. Near-surface humidity is
not provided directly in ERA5 dataset, but from near-surface ($2\,m$ from the surface) temperature ($T_{ERA5}$) and dew
...
\cite{Buck_1981}.  Zero wind speed values were recorded whenever snow accumulated on the ultrasonic wind sensor. It was
...
\%).}
    ...
    \begin{tabularx}{\linewidth}{ X l X l X  } \hline Constant Parameters & Symbol & Value & & References \\ \hline Van
      ...
      \citeauthor{Zhou_2010}\\
        ...
        \citeauthor{OerlemansKnap_1998}  \\ Surface layer thickness & $\Delta x$ & 20 $\,mm$ & $\pm$ 5 \%   & assum.\\
        % \hline Fountain Parameters& & & Range   & \\ \hline 
        % Spray Radius & $r_{spray}$ &  & $\pm$ 5 \%   & \\
        Water temperature & $T_{water}$ & 1$\degree C$ & $[0,5]\, \degree C$   & B:assum. ; U:meas. \\
        \hline \end{tabularx} \label{tab:parameters} \end{table} 
