...
The surface roughness, $z_{0}$, and surface layer thickness, $\Delta x$, may need more attention. Literature values
for surface roughness rarely exceed 10 $mm$, and for glacier ice are generally in the range $0.1-5\, mm$
\citet{BrockWillisSharp_2006}. We decided to use the maximum possible range for $z_{0}$. Since the surface layer
...
Fig. \ref{fig:results} shows the model ice volume estimates with corresponding unncertainties caused by the
parameters and the fountain.

Two main decisions explain the observed ice volume magnitude and variation namely, the construction location chosen
and the fountain used. The construction location chosen determines the weather and the fountain usage is
characterized by its four attributes.
...
water temperature ($q_{F}$) and its influence on albedo ($q_{SW}$). Considering the high correlation of surface
area on the ice volume estimates ($r^2=0.6$), we investigate the fountain influence through its impact on the AIR
surface area henceforth.

Fig. shows the estimated and observed surface area via the model and UAV respectively.
