...
We used a polynomial chaos expansion approach (as in \cite{uncertainpy_2018}; \cite{Xiu_2005}) to valuate the model
sensitivity and uncertainty.  Polynomial chaos expansion are a much more efficient way to obtain similar results
compared to the computationally demanding Monte Carlo methods. This approach approximates the model with a
...
and input. For the model parameters, we first fix a range based on literature values and then perform a global sensitivity analysis
...
for surface roughness rarely exceed 1 cm, and for glacier ice are generally in the range $0.1-5\, mm$
