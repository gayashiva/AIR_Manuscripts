...
Consider our model $U$ that depends on time $t$, has $d$ uncertain parameters $Q = [Q_1, Q_2, \dots, Q_d]$, and
gives output $Y$:
...
The output $Y$ can have any value within the output space $\sum_{Y}$ and has an unknown probability density function
$\rho_Y$. The goal of our uncertainty quantification is to describe the unknown $\rho_Y$ through statistical metrics.

We assume our model includes parameters that can be each described by a uniform probability density function
$\rho_{Q_i}$ and that the uncertain parameters are statistically independent. The joint multivariate probability
density function for the independent uncertain parameters is then:

\begin{equation}
\rho_Q = \prod_{i=1}^{d} \rho_{Q_i}
\end{equation}

% \subsubsection{Uncertainty Quantification}

A useful metric is the $(100 \cdot x)$-th percentile $P_x$ of $Y$, which defines a value below  which $100 \cdot
x$ percent of the model outputs are located. We can combine two percentiles to create a prediction interval
$I_x$, which is a range of values within which a $100 \cdot x$ percentage of the outputs $Y$ occur: 

\begin{equation}
I_x = [P_{(x/2)}, P_{(1-x/2)}]
\end{equation}

The $90\,\%$ prediction interval gives us the interval within which $90\,\%$ of the $Y$ outcomes occur, which
also means that $5\,\%$ of the outcomes are above and $5\,\%$ are below this interval.
...
sensitivity index $S_i$ measures the direct effect each parameter has on the variance of the model. Higher order
Sobol indices give the sensitivity due to interactions between a parameter $Q_i$ and various other parameters.
The total Sobol sensitivity $S_{T_{i}}$ includes the sensitivity of both the first-order effects, as well as the
sensitivity due to interactions between a given parameter $Q_i$ and all combinations of the other
parameters(Homma and Saltelli). The sum of the total Sobol sensitivity indices is equal to or greater than one,
and is only equal to one if there are no interactions between the parameters (Glen and Isaacs). Our goal is to
use sensitivity analysis to fix parameters with low sensitivity, so it total-order Sobol indices are an
appropriate metric.
...
\begin{equation} U \approx \hat{U}(t, Q) = \sum_{n=0}^{N_p-1} c_n(t) \phi_n(Q) \end{equation}
...
The uncertainty in the model of estimating ice volumes are caused due to two sources, namely, the weather and
the fountain parameters. For the weather parameters, we first fix a range based on literature values and then
perform a global sensitivity analysis with the net water loss as the objective. The distribution is always
treated as uniform and the limits for every parameter are given in Table \ref{tab:parameters}. The sensitivity
analysis consists of a total ensemble size of 992 simulations per AIR. The parameter sensitivity results from
the sensitivity analysis are used as a tool to reduce the number of free parameters in the model by removing
those parameters, which have only a marginal influence on the model output.
...
sites was just a best guess, based on few observations made by the flowmeter. So we associate this parameter by
a large uncertainty of $\pm \,50\, \%$. For the fountain water temperature, we assumed an upper bound of $3\,
\degree C$ since it is unlikely for it to have been beyond this range considering winter conditions at all the
sites.
...
After identifying the sensitive parameters, they were calibrated based on two objectives, namely, the RMSE
between the drone surveys (see Table \ref{tab:uav}) and the model estimations of the ice volume and area. For
calibration, these sensitive parameters were varied across their ranges (defined in Table \ref{tab:parameters}).
. The frequency distribution of RMSE among the best 10 \% calibration runs for each objective . We choose values
of sensitive parameters populating more than 50 \% of the possibilities among the best 10 \% of the calibration
runs.
...
thickness change was negative ($-3\, mm \,w.e.$) with a daily mean growth rate of $6\, mm \,w.e.$ and a mean decay
