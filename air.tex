...
where $q_{surf}$ is the surface energy flux in [$W\,m^{-2}$]; $q_{SW}$ is the surf shortwave radiation; $q_{LW}$ is the surf
...
% \begin{align}
%     \label{eq:vapa} 
%     \frac{\Delta M_{dep}}{\Delta t} &= \frac{\Delta M_{vapour}}{\Delta t} \textit{ if } \frac{\Delta M_{vapour}}{\Delta t} >
%     0\\
%     \label{eq:vapb} 
%     \frac{\Delta M_{sub}}{\Delta t} &= \frac{\Delta M_{vapour}}{\Delta t} \textit{ if } \frac{\Delta M_{vapour}}{\Delta t} <
%     0
% \end{align}
% \end{subequations}
...
% measured in [$kg\,s^{-1}$]; $M_{ppt}$ is the cumulative precipitation;  $M_{cdt}$ is the cumulative accumulation through
% water vapour condensation; $M_{ice}$ is the cumulative mass of ice; $M_{water}$ is the cumulative mass of melt water and
% $M_{vapour}$ represents the cumulative water vapor loss by evaporation or sublimation. 
% 
% Precipitation input is calculated as:

% Using the melt energy $q_{melt}$, we estimate the frozen and melted ice mass ($\Delta M_{ice}$, $\Delta M_{water}$).
...
% 
% \begin{equation} \textit{Storage Efficiency} = \frac{M_{water}}{(M_F+M_{ppt})} \cdot 100 \end{equation}
