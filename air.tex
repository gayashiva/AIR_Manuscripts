...
\begin{abstract}
	...
	local weather influences of the construction location chosen. This study compared the ice volume evolution of
	AIRs built in Ladakh, India and Guttannen, Switzerland using a surface energy balance model.  Model
...
The radius $r_{cone}$ evolves as follows:

\begin{equation}
	r_{cone} = r_{cone}^{i-1} + \Delta y
	\label{eq:dy}
\end{equation}

where $\Delta y$ is the rate of radius growth and is calculated as follows:

\begin{subequations}
	% \label{equations}
	\begin{align}
		\label{eq:A}
    \Delta A &= \pi \cdot (2 \cdot r \cdot \Delta y + \Delta y ^2) \\
		\label{eq:V}
    \Delta V &= 2 \cdot \pi \cdot r \cdot \Delta y \cdot \Delta z \\
		\label{eq:thickness}
    \Delta M &= \Delta V \cdot \rho_{water/ice} \\
    EB &= \Delta M \cdot L_{f}/\Delta t  \\
    \Delta y &= \sqrt{\frac{|q_{freeze/melt}| \cdot \Delta t}{2 \cdot \pi \cdot L_{f} \cdot \rho_{water/ice} \cdot r_{cone}^{i-1}}} \\
	\end{align}
\end{subequations}

The initial radius of the AIR is assumed to be $r_0$. The initial height $h_0$ depends on the dome volume
...
area of two ice cones merged into one (see Fig.  \ref{fig:2AIR}). This hypothesis was validated by the disjoint
...
to the "mass balance" of a glacier with units $m \, w.\, e.$ but it was not conserved during the lifetime of the
...
locations. It increased and decreased the freezing and melting EBC during the accumulation and ablation period
...
lower latitude. However, the total energy turnover of shortwave radiation was two times lesser for the Indian
...
The fountain had some influence on the energy fluxes through its water temperature ($q_{F}$), temperature
forcing and albedo forcing ($q_{SW}$). However, this influence was insignificant compared to its influence on the
...
The net water losses of IN21 and CH21 AIR were $78\,\%$ and $77\,\%$, respectively. The high water losses was
caused mostly due to the fountain water runoff in both the AIRs . Since vapour losses were negligible compared to
fountain runoff losses, the AIR wastes water mostly during the accumulation period. The maximum freezing rate of
the IN21 AIR was less than half the mean fountain discharge. The CH21 AIR was able to attain the mean fountain
...
The freezing rates were much higher compared to the melting rates for the Indian location but these rates have a
