...
\begin{abstract}
	...
	an AIR) to estimate the quantity of frozen, melted, evaporated and runoff water for two sites in Switzerland and
	one in India. At these measurement sites, AIRs were built for model calibration and validation purposes. The
	maximum ice volume of the Indian AIR was 8 times larger compared to the Swiss because of its favourable location
	(much lower humidity) and better fountain (higher spray radius).  However, the storage efficiency of the Indian
	AIR (15 \%) was actually lower than the Swiss AIR (~20 \%) due to significantly more fountain water runoff.
...
sensitive ($S_{T_{i}} > 0.2$) and non-sensitive ($S_{T_{i}} \leq 0.2$).

For all the AIR, just three parameters were sensitive namely, $z_{0}$, $A_{corr}$ and $\Delta x$. Model simulations
were carried out varying all the sensitive parameters within the ranges defined in Table \ref{tab:parameters}. The
model performance was expressed by the RMSE determined between the UAV ice volume observations (see Table
\ref{tab:uav}) and the model ice volume outputs. Fig.  \ref{fig:param_hist} shows the frequency distribution of the
sensitive parameters among the best 10 \% runs from a total of 275 model runs for each AIR.
...
In general, the sensitive parameters don't exhibit significant preference to any value in their ranges except the
surface layer thickness. IN21 AIR exhibited a strong preference for a $\Delta x$ of $20\,mm$. Since, this factor
was a model hyperparameter common to all the AIRs, this was set as the calibrated value. Both the AIRs prefer an
area correction factor and surface roughness in the range of 1.2-1.4 and 1-3 $mm$ respectively, so it was set as
the calibrated range and the calibrated value was the corresponding median. The rest of the parameters were
calibrated to their median values.

The uncertainty in the ice volume estimates caused by the model parameters are shown in Fig.
\ref{fig:results}.
...
snowfall is calculated directly form the precipitation quantity and sublimation/deposition quantities correspond to
...
% One of the most notable difference between the freezing and melting energy fluxes between the two locations was
% that the melting energy was negligible throughout the freezing period for the Indian site whereas the Swiss site
% melted during some days of the freezing period(e.g. day 12).

The largest contributor to the melting energy flux was the sensible heat flux for both the locations. The variance
in this energy flux  was higher the Indian AIR melted gradually but the Swiss AIR melted rapidly on certain days (e.g.
the Indian AIR melted gradually but the Swiss AIR melted rapidly on certain days (e.g.
day 140) due to the well known foehn events at the Guttannen site (Reference). The Swiss AIR has twice the standard
deviationhese foehn events were the cause of
melting even during the freezing period (e.g. day 12). So the Indian location barely produced meltwater during the
fountain runtime whereas the Swiss location was producing meltwater in the day even during fountain runtime.
...
(see Table \ref{tab:Observations}).  Since the Indian location has mostly clear days, its diffuse shortwave radiation was
...
\begin{table}
	...
	\begin{tabular}{@{}|llllll|@{}}
		...
		\multicolumn{1}{|l|}{\multirow{4}{*}{\rotatebox[origin=c]{90}{Charecteristics}}}
		                       ...
		                       & Freezing rate           & $m_{freeze}$    & $48 \pm 22$   & $5 \pm 6$     & $l/min$     \\
		\multicolumn{1}{|l|}{} & Melting rate            & $m_{melt}$      & $6 \pm 16$    & $3 \pm 10$    & $l/min$     \\
		\multicolumn{1}{|l|}{} & Storage Efficiency      & SE              & 15            & 22            & \%          \\
		\multicolumn{1}{|l|}{} & Root mean squared error & RMSE            & 84            & 10 $m^{3}$    & $m^{3}$     \\\bottomrule
	\end{tabular}
\end{table}
...
The freezing rate is bounded by the mean discharge rate, $d_F$. The Indian AIR was limited by this discharge rate
for \% of the the fountain runtime whereas the Swiss fountain only suffered for \% of it runtime.

The location determines the freezing/melting energy. The Indian location was favourable because the freezing energy
was around 2 times larger in magnitude than the melting energy. This enabled the fountain determined surface area
to favor freezing over melting. However, the Swiss location was not favourable since the freezing energy was lower
than the melting energy, indicating that the surface area did favor the melting over the freezing process.

Thus, the roughly 4 times higher surface area of the fountain and 2 times higher freezing energy of the Indian
location result in more than 8 times higher Indian AIR ice volume compared to the Swiss AIR even though the Swiss
...
The results of this analysis suggest that drier locations (e.g. Ladakh, Indian Himalayas) are significantly more
favourable since their freezing and melting rates are augmented and dampened by sublimation respectively.

Storage efficiency of all the AIR are poor but can be optimized significantly through control of the fountain
discharge rate.  The model results indicate that more than 80\% of the fountain water is lost as runoff for all the
...
\subsection{UAV observations} \label{sec:uav}
...
(3) Digital elevation model (DEM) and orthomosaic generation: DEM and orthomosaic are the two main final products.
DEM can be built from dense point cloud or irregular network, and the former one usually has higher accuracy for
rugged terrain. Every image pixel is projected on DEM and the georeferenced orthomosaic is generated
(\cite{Kung_2011}).

% During this process, we found photos with too much snow coverage that cannot be matched, that is, few feature
% points can be detected and matched in these photos, especially in survey 2 of CH20, where almost half of the photos
% are invalid (Table).
