...
\begin{abstract}
	...
	local weather influences of the construction location chosen. This study compares the evolution of AIRs built
	in Ladakh, India and Guttannen, Switzerland based on the influence of different surface processes responsible
	for their spatial and temporal ice volume changes using a surface energy balance model.  Model input
	consisted of meteorological data in conjunction with fountain discharge rate (mass input of an AIR).  Model
	calibration and validation were performed through ice volume and area measurements taken from several Uncrewed
	Aerial Vehicle (UAV) flights.  The model achieved an RMSE within $11 \%$ of the maximum ice volume with the
	UAV ice volume observations for all the three AIRs. The location in Ladakh, with a maximum ice volume around 6
	times larger, was more favourable compared to the Guttannen site.  However, the corresponding water losses for
	all the AIRs were $>75 \%$ of the total fountain discharge due to high fountain water runoff. Drier and colder
	locations in relatively cloud free regions are expected to produce AIRs with a faster freezing rate and slower
	melting rate respectively. This is a promising result for dry mountain regions, where AIR technology could
	provide a relatively cheap and sustainable strategy to alleviate climate change induced water stress.
...
losses during their lifetimes (see Appendix \ref{sec:ladakhloss}). Particularly, during their accumulation period,
...
circumference measured in the drone flights during the fountain runtime.
...
Several uncrewed aerial vehicle (UAV) surveys were conducted on the Swiss and Indian sites. The DEMs generated
through these flights were analysed to obtain the radius, area and volume of the ice structure. The number of
flights available  for the IN21, CH21 and CH20 AIR were 6, 8 and 2 respectively (see Table \ref{tab:uav}). The first drone flight was
used to set the dome volume ($V_{dome}$) for model initialisation. The remaining flights were used for model calibration and
...
\begin{subequations}
	...
	\begin{align}
		...
		\label{eq:thickness}
		t_{cone}^i & =\frac{\Delta M_{ice}^i}{\rho_{water}* A_{cone}^i}
	\end{align}
\end{subequations}

where $r_{cone}^i$ is the radius; $h_{cone}^i$ is the height; $A_{cone}^i$ is the surface area; $A_{corr}$ is a
correction factor with values between 1 and 2 that accounts for the deviation in AIR surface area from that of
the modelled conical surface; $V_{cone}^i$ is the volume and $t_{cone}$ is the AIR surface normal thickness
...
\begin{figure}
	...
	\end{center}
	\caption{Shape variables of the AIR. $r_{cone}$ is the radius, $h_{cone}$ is the height, $t_{cone}$ is the
...
temperature change processes respectively.

We categorize every model time step as freezing or melting events.  We define the total energy available to be
redistributed for melting, freezing and surface temperature change processes as $q_{total}=\rho_{ice} \cdot
	c_{ice} \cdot \frac{T_{temp}-T_{ice}}{\Delta t} \cdot \Delta x$.

Freezing events can only occur, if fountain water is available and $T_{temp}$ is below $0\,\degree C$. flux is
However, these two conditions are not sufficient as the latent heat energy can only contribute to temperature
fluctuations. Therefore, for preventing latent heat energy from turning a melting event into a freezing event an
additional condition namely, $(q_{total}-q_{L}) < 0$, is required.
...
\begin{equation}
	(q_{freeze}, q_{T}) = \left\{ \begin{array}{ll}
		(q_{total}-q_{L}+q_{0}, q_{L}-q_{0}) & \textit{ if } \Delta M_{F} \geq -\frac{(q_{total}-q_{L}+q_{0}) \cdot A_{cone} \cdot \Delta
		t}{L_f}                                                                                                                           \\
		(\frac{\Delta M_{F} \cdot L_f
		}{A_{cone} \cdot \Delta t}
		, q_{total}+\frac{\Delta M_{F} \cdot L_f
		}{A_{cone} \cdot \Delta t})          & \textit{ if } \Delta M_{F} < -\frac{(q_{total}-q_{L}+q_0) A_{cone} \cdot \Delta
		t}{L_f}
	\end{array} \right.
\end{equation}
...
and then perform a global sensitivity analysis with the net water loss ($net water losses$) as the objective. The
...
The focus of the sensitivity analysis is not on the absolute sensitivity towards single parameters, but rather to reduce the
...
namely, $z_{0}$, $A_{corr}$ and $\Delta x$.
...
objectives. We expect the UAV measured surface area of the AIRs to be higher than the modelled ice area due to
the additional surface area caused by its various ice features. Particularly, we expect the IN21 surface area
correction factor to deviate significantly from that of CH21 AIR because the IN21 surface area represents the
area of two ice cones merged into one (see Fig.  \ref{fig:2AIR}). This hypothesis is validated by the disjoint
frequency distribution of the area objective of the CH21 and IN21 AIR. Hence, we calibrate the IN21 area
correction factor to 1.5. Since no strong preference is observed for the CH21 area correction factor, we
calibrate it to the median of the values it assumed in the area objective, namely, 1.2.
...
thickness change ($t_{cone}$) as a measure to quantify the location influence. Note that $t_{cone}$ is similar
to the "mass balance" of a glacier with units $m \, w.\, e.$ but it is not conserved during the lifetime of the
AIR since the surface area varies unlike that of a glacier.  Similarly, we divide the simulation duration of the
AIR into accumulation and ablation periods. The accumulation (ablation) period ends (starts) at the last
fountain discharge event. The thickness rate during the accumulation and ablation period is referred to as
...
contribute -42 \% and -13 \% to the freezing and melting energies for the Indian location and -22 \% and -6 \%
...
freezing EBC, melt thickness corresponds to melting EBC available and the net thickness represents the surface
EBC available.  Snow deposition is calculated directly from the precipitation quantity and the
...
since the rising air temperature increases the incoming longwave radiation in the ablation period. The magnitude
...
lower latitude. However, the total energy turnover of shortwave radiation is two times lesser for the Indian
site compared to the Swiss. Moreover, there is no significant difference in magnitude of the net shortwave
radiation absorbed by both the AIRs (see Table \ref{tab:turnover}). This is because the higher diffuse shortwave
radiation at the Swiss location is compensating for its lower direct shortwave radiation . Since the IN21 site
has mostly clear days, its diffuse shortwave radiation was strongly reduced (see Table \ref{tab:Observations}).
...
\begin{table}
	...
	\begin{tabular}{@{}|llllll|@{}}
		...
		\multicolumn{1}{|l|}{} & RMSE with UAV ice volume        &                 & 86            & 14            & $m^{3}$ \\
...
radiation and diffuse shortwave radiation during the accumulation period was much less than that of the Swiss
...
forcing and albedo forcing ($q_{SW}$). However, this influence is insignificant compared to its influence on the
surface area, which has a high correlation with the ice volume ($r^2=0.6$). As seen from the uncertainty
analysis, ice volume estimates were most sensitive to the fountain spray radius parameter. The variance of this
surface area is quite low in the accumulation period since the ice radius is initialised and bounded by the
spray radius. So the thickness growth rate is uniformly scaled to produce the corresponding ice volume. The
higher spray radius of the Indian fountain resulted in a higher maximum ice volume but this was at the expense
of a faster melt-out date. This is because the fountain determined spray radius increases both the freezing and
the melting rate.  Moreover, the fountain parameters are not independent, since fountain height (ignored in this
analysis), discharge rate and spray radius are related through the projectile motion followed by the water
droplets. So a proper optimization of the fountain is much more complex and requires a closer look at the
correlation of the fountain parameters amongst themselves and with the freezing/melting energy flux. This will
be investigated in a follow up study, with this study focusing on the weather aspects of the model.

\section{Discussion}

\subsection{Water losses of AIRs}

The net water losses of IN21 and CH21 AIR were $78\,\%$ and $77\,\%$, respectively. The high water losses is caused mostly
...
Some weather attributes play a significant role in making the Indian AIR larger and survive longer than the
...
\begin{figure}
	...
	\end{center}
	\caption{Digital elevation map of Indian AIR constructed from the UAV flight on March 3, 2021. The green
