...
\title[Artificial Ice Reservoirs]{Estimating water storage efficiency of artificial ice reservoirs (Icestupas) in the Swiss Alps and Indian Himalayas}
% \title[Artificial Ice Reservoirs]{Environmental influences on water storage efficiency of artificial ice reservoirs (Icestupas)}
% \title[Modelling Artificial Ice Reservoirs]{Modelling artificial ice reservoirs (Icestupas) through an energy balance approach}
% \title[Artificial Ice Reservoirs]{Mass and energy balance calculations for artificial ice reservoirs (Icestupas)}
% \title[Artificial Ice Reservoirs]{Water storage potential of artificial ice reservoirs (Icestupas) in different climate regimes}
% \title[Artificial Ice Reservoirs]{Modelling water storage capacity of artificial ice reservoirs (Icestupas) in different climate regimes}
% \title[Artificial Ice Reservoirs]{ A new metric to rank prospective artificial ice reservoir (Icestupa) locations
%     through an energy balance model}
...
Artificial Ice Reservoirs (AIR) can successfully store water during winter and release the water during spring and
summer. This makes them a reliable fresh water resource for irrigation. Several AIRs have been built worldwide but
studies of their water storage capacity or efficiency are scarce. This study models a cone-shaped AIR popularly called
Icestupa.  Processes involved in the development and temporal evolution of an Icestupa are calculated by a
...
sites in Switzerland and one in India. At these measurement sites, AIR were built for model calibration and validation
purposes. Model estimates had a normalised root mean square error of 5 \% and a correlation of 0.9. Water storage
efficiency of the AIR varied widely not just between different locations but also between different winters.
...
% in winter \citep{Chevuturi_2018}.
...
\multicolumn{1}{|l|}{\multirow{4}{*}{\rotatebox[origin=c]{90}{AIR}}} 
                       ...
                       \multicolumn{1}{|l|}{} & Ice Volume measurements     &[times]& 6   & 9    & 3      \\
...
We define the first term in Eqn. \ref{eq:mice} as the fountain discharge freezing rate $d_{ice}$ since it represents
...
derived from literature values and a discrete step .  Table \ref{tab:parameters} shows the optimized parameters at all
AIR. The results point to considerably higher albedo values for indian compared to swiss sites. This corresponds to
visual observations, since swiss AIR 
...
Ice Albedo ($\alpha_{ice}$)                     & (0.01-0.35, 0.05)                    & 0.25        & 0.24  &  &  \\
...
Surface Roughness ($z_0$)                       & (1-5, 1) $mm$                        & $1.7\,mm$     & $1\,mm$ &  &  \\ 
Surface layer thickness ($\Delta x$)            & (19-21, 0.5) $mm$                    & $20\,mm$    & $20\,mm$ &  &  \\
Water temperature ($T_{water}$)                 & (0-3, 0.5) $\,\degree C$              & $0.5\degree C$&$1\degree C$  &  &
\\\bottomrule
% \textbf{Fountain parameters} & &  & & &  \\\midrule
%      Spray Radius ($r_F$)        &                                & meas.      &$6.9\,m$  &$10.8\,m$  &$7.7\,m$  \\ 
%      Mean discharge ($d_F$)       &                                & meas.      &$7.5\,l/min$  &$60\,l/min$  &$7.5\,l/min$  \\\midrule 
\end{tabular}
\end{table}

\subsection{Mass and Energy fluxes of the AIR}

Fig. \ref{fig:MEB} shows the daily averages of the mass and energy fluxes calculated with optimal parameters for the
...
and sublimation processes for all the AIR. This is a striking result and the first quantification of the role played by
...
importantly, this fountain contribution varies significantly in magnitude between the Swiss and the Indian AIR, since
the higher surface area (which is a function of the fountain spray radius) of the Indian AIR compounds this influence.
As can be seen in Table \ref{tab:Results}, the corresponding fountain freezing rate for the Indian AIR was more than 5
times more than the Swiss AIR.  This higher fountain freezing rate resulted in ice volumes 4 times higher for the Indian
compared to the Swiss AIR even though the fountain duration of the Indian AIR was just around half that of the Swiss
AIR. 

Incoming shortwave radiation is a major source of melting energy for all the AIR. Its variability is controlled by the
...
Turbulent fluxes vary significantly in magnitude and distribution between the two time periods and the two locations. In
the freezing period, latent heat is insignificant as the vapor flux between the AIR surface and the atmosphere is
minimal.  Sensible (Latent) heat drive the melting (freezing) of the Indian AIR compared to the Swiss AIR especially
during the melting period. This is primarily because of the high wind speeds. Particularly, the daily melt rate of the
Indian AIR is slowed down as the higher sensible heat is balanced by the latent heat. This occurs due to the sublimation
process favoured by the high vapour fluxes generated between the melting AIR surface and the dry air surrounding it. 

% However it varies both in magnitude and distribution between the different locations, since the indian AIR is much
% higher in altitude and has mostly clear days (Ref.) resulting in much higher shortwave radiation with a negligible
% diffuse component (Ref.).
...
\multicolumn{1}{|l|}{} & Mean fountain freezing rate      & $(d_{ice})_{mean}$& 11 $l/min$  & 2 $l/min$ & 2 $l/min$ \\\bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{@{}|llllll|@{}}
\toprule
\textbf{}              & \textbf{Name} & \textbf{Symbol [Units]}      & \textbf{IN21} & \textbf{CH21} & \textbf{CH20} \\ \midrule
...
\multicolumn{1}{|l|}{} & Freezing energy    &$q_{freeze} [W\,m^{-2}] $&      $-34 \pm 69$&     $-26 \pm 45$&     $-29\pm 43$\\
\multicolumn{1}{|l|}{} & Melting energy     &$q_{melt} [W\,m^{-2}] $&      $17 \pm 48$&     $47 \pm 103$&     $40 \pm 76$\\
\multicolumn{1}{|l|}{} & Temperature        &$q_{T} [W\,m^{-2}] $&      $0 \pm 87$&     $0 \pm 37$&     $0 \pm 23$\\\bottomrule
\end{tabular}
\end{table}
...
area and fountain discharge energy flux allowed an accurate representation of the complex growth dynamics typical of any AIR. The model
was calibrated by comparison with the ice volume observations logged via drone flights. We calculated the storage
efficiency for each of the three AIR and used calculations of the energy fluxes to explain the observed variability in
freezing and melting rates. We also looked at the model sensitivity to model parameters. Our main conclusions are
summarised below.

Freezing rates were driven by the energy flux due to AIR surface temperature changes while fountain was on. 

Freezing period and melting period exhibited large variability in fluxes.

Observed freezing rates between the Swiss and the Indian AIR exhibited large variability.

Observed melting rates for the Indian AIR was significantly lower than the Swiss AIR.
